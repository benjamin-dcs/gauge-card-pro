  ...
  implements LovelaceCardEditor
{
  ...
  protected render() {
    ...
    const innerMinIndicatorType = this._config.inner?.min_indicator?.type ?? undefined;
    const innerMaxIndicatorType = this._config.inner?.max_indicator?.type ?? undefined;
  ...
  private _valueChanged(ev: CustomEvent): void {
    ...
    // Inner Min indicator
    if (config.inner?.min_indicator?.type !== this._config?.inner?.min_indicator?.type) {
      config = deleteKey(config, "inner.min_indicator.value").result;
    }

    // Inner Max indicator
    if (config.inner?.max_indicator?.type !== this._config?.inner?.max_indicator?.type) {
