 ...
 */ function interpolateHsv(stop1, stop2, steps, mode) {
  ...
  if (typeof mode === "boolean") {
    ...
    const trigShortest =
      (start.h < end.h && end.h - start.h < 180) ||
      (start.h > end.h && start.h - end.h > 180);
    trigonometric =
      (mode === "long" && trigShortest) || (mode === "short" && !trigShortest);
  }
  const step = stepize(start, end, steps);
  let gradient = [stop1.color]; // recompute hue
  let diff;
  if (
    (start.h <= end.h && !trigonometric) ||
    (start.h >= end.h && trigonometric)
  ) {
    diff = end.h - start.h;
...
class TinyGradient {
   ...
   */ constructor(stops) {
    ...
    if (this.stops[l - 1].pos !== 1) {
      this.stops.push({ color: this.stops[l - 1].color, pos: 1 });
    }
  }
  /**
   ...
   */ reverse() {
    ...
    this.stops.forEach((stop) => {
      stops.push({ color: stop.color, pos: 1 - stop.pos });
    });
   ...
   */ loop() {
    ...
    let stops2 = [];
    this.stops.forEach((stop) => {
      stops1.push({ color: stop.color, pos: stop.pos / 2 });
    });
    this.stops.slice(0, -1).forEach((stop) => {
      stops2.push({ color: stop.color, pos: 1 - stop.pos / 2 });
    });
   ...
   */ css(mode, direction) {
    mode = mode || "linear";
    direction =
      direction || (mode === "linear" ? "to right" : "ellipse at center");
...
const styleAttrs = (fill, stroke, strokeWidth, progress) => {
...
const DEFAULT_PRECISION = 2; // The main function responsible for getting data
...
const getData = (_ref) => {
  let { path, segments, samples, precision = DEFAULT_PRECISION } = _ref; // Convert the given path to a DOM node if it isn't already one
  ...
  for (let sample = 0; sample <= totalSamples; sample++) {
    ...
    let { x, y } = path.getPointAtLength(progress * pathLength); // If the user asks to round our x and y values, do so
    if (precision) {
...
const strokeToFill = (data, width, precision, pathClosed) => {
...
const averageSegmentJoins = (outlinedData, precision, pathClosed) => {
  ...
  for (let i = 0; i < outlinedData.length; i++) {
    ...
    const currentSamples = pathClosed
        ...
        : outlinedData[i + 1]
          ? outlinedData[i].samples
          : init_outlinedData[i].samples, // If path is closed: the next segment's samples, otherwise, the first segment's samples
    ...
    nextSamples[0] = new Sample({ ...combine(nextSamples, 0, p0Average) });
  ...
  render(_ref2) {
    ...
    if (type === "path") {
      ...
      for (let j = 0; j < renderCycle.data.length; j++) {
        const { samples, progress } = renderCycle.data[j]; // Create a path for each segment and append it to its elemGroup
      ...
      renderCycle.data = this.data.flatMap((_ref3) => {
        let { samples } = _ref3;
        return samples;
      });
      for (let j = 0; j < renderCycle.data.length; j++) {
        const { x, y, progress } = renderCycle.data[j]; // Create a circle for each sample and append it to its elemGroup
        elemGroup.appendChild(
          svgElem("circle", {
            class: "circle-sample",
            cx: x,
...
export { GradientPath, getData, strokeToFill };
